...
public class DrugsUtilizationController {
    private static final Logger logger = LoggerFactory.getLogger(DrugsUtilizationController.class);


    @Autowired
    private DrugsUtilizationService drugsUtilizationService;
    ...
    */
    ...
    public R page(@RequestParam Map<String, Object> params, HttpServletRequest request){
        ...
        PageUtils page = drugsUtilizationService.queryPage(params);

        //字典表数据转换
        List<DrugsUtilizationView> list =(List<DrugsUtilizationView>)page.getList();
        for(DrugsUtilizationView c:list){
            //修改对应字典表字段
    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        DrugsUtilizationEntity yaopinshiyong = drugsUtilizationService.selectById(id);
        if(yaopinshiyong !=null){
            //entity转view
            DrugsUtilizationView view = new DrugsUtilizationView();
            BeanUtils.copyProperties( yaopinshiyong , view );//把实体数据重构到view中
            //级联表 病人
            //级联表
            PatientEntity bingren = patientService.selectById(yaopinshiyong.getBingrenId());
            if(bingren != null){
            ...
            //级联表 药品
            //级联表
            DrugEntity yaopin = drugService.selectById(yaopinshiyong.getYaopinId());
            if(yaopin != null){
            ...
            //级联表 医生
            //级联表
            DoctorEntity yisheng = doctorService.selectById(yaopinshiyong.getYishengId());
            if(yisheng != null){
    ...
    */
    @RequestMapping("/save")
    public R save(@RequestBody DrugsUtilizationEntity yaopinshiyong, HttpServletRequest request){
        ...
        Wrapper<DrugsUtilizationEntity> queryWrapper = new EntityWrapper<DrugsUtilizationEntity>()
        ...
        DrugsUtilizationEntity yaopinshiyongEntity = drugsUtilizationService.selectOne(queryWrapper);
        if(yaopinshiyongEntity==null){



            DrugEntity yaopinEntity = drugService.selectById(yaopinshiyong.getYaopinId());
            ...
            drugsUtilizationService.insert(yaopinshiyong);
            return R.ok();
        }else {
    ...
    */
    ...
    public R update(@RequestBody DrugsUtilizationEntity yaopinshiyong, HttpServletRequest request) throws NoSuchFieldException, ClassNotFoundException, IllegalAccessException, InstantiationException {
        logger.debug("update方法:,,Controller:{},,yaopinshiyong:{}",this.getClass().getName(),yaopinshiyong.toString());
        DrugsUtilizationEntity oldYaopinshiyongEntity = drugsUtilizationService.selectById(yaopinshiyong.getId());//查询原先数据
...
//            yaopinshiyong.setYishengId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
            ...
            drugsUtilizationService.updateById(yaopinshiyong);//根据id更新
            return R.ok();
    }
    ...
    */
    ...
    public R delete(@RequestBody Integer[] ids, HttpServletRequest request){
        ...
        List<DrugsUtilizationEntity> oldYaopinshiyongList = drugsUtilizationService.selectBatchIds(Arrays.asList(ids));//要删除的数据
        drugsUtilizationService.deleteBatchIds(Arrays.asList(ids));
    ...
    public R save( String fileName, HttpServletRequest request){
        ...
        try {
            List<DrugsUtilizationEntity> yaopinshiyongList = new ArrayList<>();//上传的东西
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            DrugsUtilizationEntity yaopinshiyongEntity = new DrugsUtilizationEntity();
...
//                            yaopinshiyongEntity.setCreateTime(date);//时间
                        ...
                        //查询是否重复
                         //药品使用编号
                        List<DrugsUtilizationEntity> yaopinshiyongEntities_yaopinshiyongUuidNumber = drugsUtilizationService.selectList(new EntityWrapper<DrugsUtilizationEntity>().in("yaopinshiyong_uuid_number", seachFields.get("yaopinshiyongUuidNumber")));
                        if(yaopinshiyongEntities_yaopinshiyongUuidNumber.size() >0 ){
                            ArrayList<String> repeatFields = new ArrayList<>();
                            for(DrugsUtilizationEntity s:yaopinshiyongEntities_yaopinshiyongUuidNumber){
                                repeatFields.add(s.getYaopinshiyongUuidNumber());
                            }
                            return R.error(511,"数据库的该表中的 [药品使用编号] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
                        drugsUtilizationService.insertBatch(yaopinshiyongList);
                        return R.ok();
                    }
                }
            }
    ...
    */
    @RequestMapping("/add")
    public R add(@RequestBody DrugsUtilizationEntity yaopinshiyong, HttpServletRequest request){
